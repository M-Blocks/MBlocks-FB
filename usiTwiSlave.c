...
#  define DDR_USI_SDA         DDRB
#  define PORT_USI_SDA        PORTB
#  define PIN_USI_SDA         PINB
#  define PIN_NUMBER_USI_SDA  1

#  define DDR_USI_SCL         DDRC
#  define PORT_USI_SCL        PORTC
#  define PIN_USI_SCL         PINC
#  define PIN_NUMBER_USI_SCL  1
...
{ \
  ...
  USIDR = 0; \
  /* set SDA as output */ \
  DDR_USI_SDA |= ( 1 << PIN_NUMBER_USI_SDA ); \
...
)
{
  ...
  DDR_USI_SCL |= ( 1 << PIN_NUMBER_USI_SCL );
  DDR_USI_SDA |= ( 1 << PIN_NUMBER_USI_SDA );

  // set SCL high
  PORT_USI_SCL |= ( 1 << PIN_NUMBER_USI_SCL );

  // set SDA high
  PORT_USI_SDA |= ( 1 << PIN_NUMBER_USI_SDA );
...
ISR( USI_START_VECTOR )
{
  ...
  while (
       // SCL his high
       ( PIN_USI_SCL & ( 1 << PIN_NUMBER_USI_SCL ) ) &&
       // and SDA is low
       !( ( PIN_USI_SDA & ( 1 << PIN_NUMBER_USI_SDA ) ) )
  );


  if ( !( PIN_USI_SDA & ( 1 << PIN_NUMBER_USI_SDA ) ) )
  {
  ...
  switch ( overflowState )
  {
    ...
    case USI_SLAVE_CHECK_ADDRESS:
      if ( ( USIDR == 0 ) || ( ( USIDR >> 1 ) == slaveAddress) )
      {
         ...
         if ( USIDR & 0x01 )
        {
          overflowState = USI_SLAVE_SEND_DATA; // master read
        }
        else
        {
          overflowState = USI_SLAVE_REQUEST_DATA; // master write
        } // end if
        SET_USI_TO_SEND_ACK( );
      }
      else
